...
function Messages(){
	...
	useEffect(()=>{
		if(messages != undefined && messages.length == 0 && !secondLoadMessages){
			dispatch(dispatchAction(ticketActions.SET_STATE, {secondLoadMessages:true}))
			dispatch(dispatchAction(ticketActions.GET_MESSAGES))
		}	
	},[])

	useEffect(() => {
		if(error && error.length > 0 && errorAction==='my-messages'){
			toast.error(error)
	...
	const getDetailTicket = (id) => {
		const array = messages.filter(ticket => ticket.id === id)
		setTicket(array[0])
	}
	...
	return(
		<div className="content flex fdc fs20 padh40">
			<div className="flex fdr jcsb padt20">
				<p>Mes Message - Tickets Effectués</p>
			</div>
